...
dst = np.array([[185, 50], [215, 50], [201, 500], [199, 500]], dtype=np.float32)
...
lower_W_line = np.array([10, 0, 200])
...
def sliding_window(img, nwindows=50, margin=10, minpix=1, draw_windows=True):
    ...
    for window in range(nwindows):
        ...
        if len(good_right_inds) > minpix:
            ...
            #rightx_current = np.int(np.mean(nonzerox[good_right_inds]))
            while len(good_right_inds) < minpix:
                #print("dfdf",nonzerox)
                win_xright_low=np.argmax(histogram[win_xright_low:]) + win_xright_low
                win_xright_high = win_xright_high + 10
                #print("sss",np.argmax(histogram[win_xright_low:]) + win_xright_low)
                good_right_inds = ((nonzeroy >= win_y_low) & (nonzeroy < win_y_high) &
                                   (nonzerox >= win_xright_low) & (nonzerox < win_xright_high)).nonzero()[0]
                ...
                if len(good_left_inds) >= minpix:
                    right_lane_inds.append(good_right_inds)
                    break
                if win_xright_high>=400:
                    #win_xright_low=400-margin
                    break
                cv2.rectangle(out_img, (win_xright_low, win_y_low), (win_xright_high, win_y_high),
                              (255, 0, 0), 3)
...
Left_img = cv2.VideoCapture("C:/Users/201921343/Desktop/[SHANA][SHANA]outputl_cut.mp4")
...
while (1):
    ...
    # print(np.shape(warped2))
    # cv2.imshow('lanesss',cv2.bitwise_and(255-warped2, L_img))
