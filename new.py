...
def undistort_img():
    ...
    pickle.dump( dist_pickle, open('C:\\Users\\201921343\\Desktop\\cal_pickle.p', 'wb') )
...
def get_curve(img, leftx, rightx):
    ploty = np.linspace(0, img.shape[0] - 1, img.shape[0])
    y_eval = np.max(ploty)
    ym_per_pix = 30.5 / 1280  # meters per pixel in y dimension
...
def draw_lanes(img, left_fit, right_fit):
    ...
    cv2.polylines(left_img, np.int_(left), False, (0, 255, 153), 100)
    cv2.polylines(right_img, np.int_(right), False, (0, 255, 153), 100)
...
def hsv_img(cap):
    ...
    lower_yellow = np.array([10, 60, 50])
    ...
    # res3 = cv2.bitwise_and(imgz, imgz, mask=mask3)

    return res1, res2
img = cv2.imread('C:\\Users\\201921343\\Desktop\\pictures\\straight_lines1.jpg')
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
dst2 = pipeline(img)
dst2 = perspective_warp(dst2, dst_size=(1280,720))
dst=undistort(img)
dst = perspective_warp(dst, dst_size=(1280,720))
yellow, blue=hsv_img(dst)

out_img, curves, lanes, ploty = sliding_window(dst2)
print(curves[0])
cv2.imshow("",out_img)
y=0
r_curve=curves[0]
