...
def hsv_img(cap):
    ...
    lower_yellow = np.array([10, 70, 50])
...
def Find_lane(img):
    ...
    yellow_img, blue_img, white_img = hsv_img(perspective_warp(imgz))
...
while(1):
    ...
    test = white_img[0:white_img.shape[0], int(lane[-1][0]):int(lane[-1][-1])]
    ...
    uio = weighted_img(asd, yellow_img)
    ert = cv2.bitwise_or(asd, yellow_img)
    cv2.imshow('asd', test)
    cv2.imshow('assd', yellow_img)
